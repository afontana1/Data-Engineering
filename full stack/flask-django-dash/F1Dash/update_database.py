...
def refresh_schedule(pyodbc_connection, sqlalchemy_engine, reload_history=False):
    ...
    last_event_date_with_data = cursor.execute(
        "SET NOCOUNT ON; EXEC dbo.Get_LastEventDateWithData"
    ).fetchval()
    schedule = schedule[
        (schedule["EventDate"] > last_event_date_with_data)
    ].reset_index(drop=True)
    schedule["id"] = 0

    new_event_id = (
        cursor.execute(
            "SET NOCOUNT ON; EXEC dbo.Get_MaxId @TableName=?", "dbo.Event"
        ).fetchval()
        + 1
    )
    new_session_id = (
        cursor.execute(
            "SET NOCOUNT ON; EXEC dbo.Get_MaxId @TableName=?", "dbo.Session"
        ).fetchval()
        + 1
    )
    ...
    session_frames = []
    for i in range(1, 6):
        session_frame = schedule[
            ["id", "Session" + str(i), "Session" + str(i) + "Date"]
        ][
            (~schedule["Session" + str(i)].isnull())
            & (~schedule["Session" + str(i) + "Date"].isnull())
        ]
        if len(session_frame) == 0:
            continue
        session_frame.rename(
            columns={
                "id": "EventId",
                "Session" + str(i): "SessionName",
                "Session" + str(i) + "Date": "SessionDate",
            },
            inplace=True,
        )
...
def load_session_data(
    ...
    for session in sessions_data:
        ...
        try:
            ...
            data_logging(
                pyodbc_connection, f"Lap data unavailable: {session['api_string']}"
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection, f"Timing data unavailable: {session['api_string']}"
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection, f"Car data unavailable: {session['api_string']}"
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection, f"Position data unavailable: {session['api_string']}"
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection,
                f"Track status data unavailable: {session['api_string']}",
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection,
                f"Session status data unavailable: {session['api_string']}",
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection,
                f"Session driver info unavailable: {session['api_string']}",
            )
            abort = True
        ...
        try:
            ...
            data_logging(
                pyodbc_connection,
                f"Session weather data unavailable: {session['api_string']}",
            )
        ...
        if not abort:
            for dataset in [
                ...
                if len(dataset[0]) == 0:
                    data_logging(
                        pyodbc_connection,
                        f"Zero-length result for {dataset[1]}: {session['api_string']}",
                    )
                    abort = True
        ...
        if abort:
            ...
            cursor.execute(
                "EXEC dbo.Update_IncrementAbortedLoadCount @SessionId=?",
                int(session["SessionId"]),
            )
            cursor.commit()
            data_logging(
                pyodbc_connection, f"Data load aborted: {session['api_string']}"
            )
            continue

        new_lapId = (
            cursor.execute(
                "SET NOCOUNT ON; EXEC dbo.Get_MaxId @TableName=?", "dbo.Lap"
            ).fetchval()
            + 1
        )
        ...
        sector_frames = []
        for i in range(1, 4):
            sector_frame = lap_data[
                [
                    "id",
                    "Driver",
                    "Sector" + str(i) + "Time",
                    "Sector" + str(i) + "SessionTime",
                ]
            ][
                (~lap_data["Sector" + str(i) + "Time"].isnull())
                & (lap_data["Driver"] != "")
            ]
            if len(sector_frame) == 0:
                continue
            sector_frame.rename(
                columns={
                    "id": "LapId",
                    "Sector" + str(i) + "Time": "SectorTime",
                    "Sector" + str(i) + "SessionTime": "SectorSessionTime",
                },
                inplace=True,
            )
        ...
        existing_counts = pd.read_sql_query(
            f"SET NOCOUNT ON; EXEC dbo.Get_TelemetryRowCounts @SessionId = {session['SessionId']}",
            sqlalchemy_engine,
        )
        existing_total = (
            existing_counts["Laps"][0]
            + existing_counts["Sectors"][0]
            + existing_counts["TimingData"][0]
            + existing_counts["CarData"][0]
            + existing_counts["PositionData"][0]
            + existing_counts["TrackStatus"][0]
            + existing_counts["SessionStatus"][0]
            + existing_counts["DriverInfo"][0]
        ...
        if new_total <= existing_total and force_reload == False:
            ...
            cursor.execute(
                "EXEC dbo.Update_SessionLoadStatus @SessionId=?, @Status=?",
                int(session["SessionId"]),
                1,
            )
            cursor.commit()
            data_logging(
                pyodbc_connection,
                f"Confirmed data load complete for SessionId {session['SessionId']}",
            )
        else:
            # Load /reload data
            cursor.execute(
                "EXEC dbo.Delete_Telemetry @SessionId=?", int(session["SessionId"])
            )
            cursor.commit()
            ...
            for dataset in [
                ...
                data_logging(
                    pyodbc_connection,
                    f"Loading {len(dataset[0])} records to {dataset[1]}",
                )
                chunk_ranges = []
                chunk_size = 100000
                abort = False
                for i in range(0, int(len(dataset[0]) / chunk_size) + 1):
                    chunk_ranges.append(
                        (i * chunk_size, min((i + 1) * chunk_size, len(dataset[0])))
                    )
                ...
                for chunk_range in chunk_ranges:
                    ...
                    if len(chunk_ranges) > 1:
                        data_logging(
                            pyodbc_connection,
                            f"Loading chunk {str(chunk_range[0])}:{str(chunk_range[1])} to {dataset[1]}",
                        )
                    while not success and error_count < 3:
                        try:
                            dataset[0].iloc[chunk_range[0] : chunk_range[1]].to_sql(
                                dataset[1],
                                sqlalchemy_engine,
                                if_exists="append",
                                index=False,
                            ...
                            data_logging(
                                pyodbc_connection,
                                f"Operational error loading chunk; attempt {str(error_count)}",
                            )
                            time.sleep(5)
                        else:
                            success = True
            ...
            if not abort:
                cursor.execute(
                    "EXEC dbo.Update_SetNullTimes @SessionId=?",
                    int(session["SessionId"]),
                )
                cursor.execute(
                    "EXEC dbo.Update_SetDriverTeamOrders @SessionId=?",
                    int(session["SessionId"]),
                )
                cursor.execute(
                    "EXEC dbo.Insert_MissingSectors @SessionId=?",
                    int(session["SessionId"]),
                )
                ...
                data_logging(
                    pyodbc_connection,
                    f"Data load at least partially complete for SessionId {session['SessionId']}",
                )
...
def run_transforms(
    pyodbc_connection, sqlalchemy_engine, force_eventId=None, force_sessionId=None
):
    ...
    while not success and error_count < 3:
        try:
            ...
            sessions_frame = pd.read_sql_query(
                "SET NOCOUNT ON; " + sql, sqlalchemy_engine
            )[["SessionId", "EventId"]]
            session_dicts = []
            for i in range(0, len(sessions_frame)):
            ...
            if len(session_dicts) > 0:
                data_logging(
                    pyodbc_connection,
                    f"Running transforms for {len(session_dicts)} sessions...",
                )
            ...
            for iSession, session_dict in enumerate(session_dicts):
                ...
                data_logging(
                    pyodbc_connection, f"Running transforms for sessionId {sessionId}"
                )

                cursor.execute(
                    "SET NOCOUNT ON; EXEC dbo.Update_SessionTransformStatus @SessionId=?, @Status=?",
                    int(sessionId),
                    0,
                )

                cursor.execute(
                    "SET NOCOUNT ON; EXEC dbo.Merge_UpdateTelemetryTimes @SessionId=?",
                    int(sessionId),
                )
                data_logging(
                    pyodbc_connection,
                    f"Ran Merge_UpdateTelemetryTimes for sessionId {sessionId}",
                )

                cursor.execute(
                    "SET NOCOUNT ON; EXEC dbo.Merge_LapData @SessionId=?",
                    int(sessionId),
                )
                data_logging(
                    pyodbc_connection, f"Ran Merge_LapData for sessionId {sessionId}"
                )

                cursor.execute(
                    "SET NOCOUNT ON; EXEC dbo.Merge_CarData @SessionId=?",
                    int(sessionId),
                )
                data_logging(
                    pyodbc_connection, f"Ran Merge_CarData for sessionId {sessionId}"
                )

                cursor.execute(
                    "SET NOCOUNT ON; EXEC dbo.Merge_TrackMap @EventId=?", int(eventId)
                )
                data_logging(
                    pyodbc_connection, f"Ran Merge_TrackMap for SessionId {sessionId}"
                )

                cursor.execute(
                    "SET NOCOUNT ON; EXEC dbo.Merge_CarDataNorms @SessionId=?",
                    int(sessionId),
                )
                data_logging(
                    pyodbc_connection,
                    f"Ran Merge_CarDataNorms for SessionId {sessionId}",
                )
                ...
                data_logging(
                    pyodbc_connection,
                    f"Completed transforms for SessionId {sessionId} ({iSession+1} of {len(session_dicts)})",
                )
            ...
            data_logging(
                pyodbc_connection,
                f"Operational error during transforms; attempt {str(error_count)}",
            )
            time.sleep(5)
        else:
            success = True
