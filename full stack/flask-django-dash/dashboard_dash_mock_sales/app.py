...
def get_sales_margin(master_df):
    try:
        return "{:,.2f}%".format(
            ((master_df["Profit"].sum()) / (master_df["Sales"].sum())) * 100
        )
...
requirement_iii = (
    master_df[["Sales", "Discount", "Cost", "Profit", "ProductCategory"]]
    .groupby("ProductCategory")
    .sum()
    .reset_index()
)

viz_scatter_iii = px.scatter(
    requirement_iii,
    x="Sales",
    y="Profit",
    size="Profit",
...
layout = viz_scatter_iii.update_layout(
    hoverlabel=dict(bgcolor="white", font_size=16, font_family="Open Sans"),
    height=700,
    width=700,
...
viz_scatter_iii.update_traces(
    mode="markers", hovertemplate="Sales: %{x} <br>Profit: %{y}"
)
...
city_sales = (
    master_df[["Sales", "Profit", "Cost", "Quantity", "Discount", "City"]]
    .groupby("City")
    .sum()
)
lat_log_city = (
    master_df[["City", "Latitude", "Longitude"]].drop_duplicates().set_index("City")
)
requirement_ii = pd.concat(
    [city_sales, lat_log_city], axis=1, join="inner"
).reset_index()
requirement_ii["Field"] = (
    " City"
    + "<br>"
    + requirement_ii["City"]
    + "<br>Sales "
        ...
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        html.Div(
                            dbc.Alert(
                                "QUANTITY " + get_sales_quantity(master_df),
                                color="light",
                            )
                        ),
                        width=2,
                    ...
                    dbc.Col(
                        html.Div(
                            dbc.Alert(
                                "EXPENSES " + get_expenses(master_df), color="light"
                            )
                        ),
                        width=2,
                    ),
                    dbc.Col(
                        html.Div(
                            dbc.Alert(
                                "DISCOUNT " + get_discount(master_df), color="light"
                            )
                        ),
                        width=2,
                    ),
                    dbc.Col(
                        html.Div(
                            dbc.Alert("PROFIT " + get_profit(master_df), color="light")
                        ),
                        width=2,
                    ),
                    dbc.Col(
                        html.Div(
                            dbc.Alert(
                                "MARGIN " + get_sales_margin(master_df), color="light"
                            )
                        ),
                        width=2,
                    ),
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [dcc.Graph(id="graph-4", figure=map_data)],
                        className="six columns",
                        style={
                            "border-radius": "15px",
                            "backgroundColor": "#f8f9fa",
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [dcc.Graph(id="graph-1", figure=viz_scatter_iii)],
                        className="six columns",
                        style={
                            "border-radius": "15px",
                            "backgroundColor": "#f8f9fa",
...
def select_cat(selector):
    ...
    revenue_country = (
        revenue_country.groupby(selector).sum().reset_index().sort_values("Sales")
    )

    revenue_country_viz = px.bar(
        revenue_country,
        y=selector,
        x="Sales",
        text="Sales",
        template="seaborn",
        hover_data={"Sales": ":.2f"},
        title="REVENUE BY " + str(selector).upper(),
    )

    revenue_country_viz.update_traces(
        texttemplate="%{text:.2s}", textposition="outside"
    )
    revenue_country_viz.update_layout(
        uniformtext_minsize=7, uniformtext_mode="hide", height=600
    )
