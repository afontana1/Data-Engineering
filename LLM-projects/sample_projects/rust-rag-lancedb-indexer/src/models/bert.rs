...
impl BertModelWrapper {
    ...
    pub fn embed_sentences(&self, sentences: &[&str], apply_mean: bool) -> anyhow::Result<Tensor> {
        ...
        let max_length = all_tokens[0].get_ids().len();
    ...
    fn cosine_similarity(a: &Tensor, b: &Tensor) -> anyhow::Result<f64> {
        ...
        let sum_i2 = (a * b)?.sum_all()?.to_scalar::<f64>()?;
        let sum_j2 = (a * b)?.sum_all()?.to_scalar::<f64>()?;
...
mod tests {
    ...
    fn test_cosine_similarity() {
        ...
        let similarity_ac = BertModelWrapper::cosine_similarity(&a, &b).unwrap();
    ...
    #[cfg(test)]
    fn test_embed_multiple_sentence() {
        ...
        let sentences_1 = vec![
            ...
            "As a Siamese, my cat has beautiful coat",
        ];

        let sentences_2 = vec![
            "My kitten enjoys playing with the string",
