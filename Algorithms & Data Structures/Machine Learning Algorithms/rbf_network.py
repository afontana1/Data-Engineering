    ...
    def fit(self, X, y, units, epochs, optimizer):
        ...
        for _ in range(epochs):
            ...
            for i in range(self.__units):
                outs[:, i] = kernel.gaussian_kernel(
                    X, self.__centers[i].reshape(1, -1), self.__sigmas[i]
                )
            ...
            for i in range(self.__units):
                ...
                g_sigmas[i] = (
                    ...
                    * np.mean(
                        residual
                        * outs[:, i]
                        * (np.linalg.norm(X - self.__centers[i], axis=1) ** 2),
                        axis=0,
                    )
                    / (self.__sigmas[i] ** 3)
                )
            ...
            g_centers, g_sigmas, g_weights = optimizer.optimize(
                [g_centers, g_sigmas, g_weights]
            )
