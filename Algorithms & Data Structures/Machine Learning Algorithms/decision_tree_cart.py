...
class CART:
    ...
    def __get_score(self, y_left, y_right):
        if self.__mode == "regression":
            ...
            return (
                y_left_number * self.__get_gini(y_left)
                + y_right_number * self.__get_gini(y_right)
            ) / (y_left_number + y_right_number)
    ...
    def __create_tree(self, parent, X, y):
        ...
        if data.threshold_split:
            ...
            node = self.__tree.create_node(
                "less " + str(data.threshold_split), parent=parent
            )
            self.__create_tree(node, X[less_items], y[less_items])

            node = self.__tree.create_node(
                "greater " + str(data.threshold_split), parent=parent
            )
            self.__create_tree(node, X[greater_items], y[greater_items])
        elif data.feature_label_split:
            left_items = np.flatnonzero(
                X[:, data.feature_split] == data.feature_label_split
            )
            right_items = np.flatnonzero(
                X[:, data.feature_split] != data.feature_label_split
            )

            node = self.__tree.create_node(
                "is " + str(data.feature_label_split), parent=parent
            )
            self.__create_tree(node, X[left_items], y[left_items])

            node = self.__tree.create_node(
                "not " + str(data.feature_label_split), parent=parent
            )
    ...
    def prune_ccp(self, X, y):
        ...
        while True:
            ...
            for node in tree.all_nodes():
                ...
                if n_leaves > 1:
                    leaves_error = sum(
                        [leaf.data.n_errors for leaf in tree.leaves(node.identifier)]
                    )
    ...
    def __query(self, x, node):
        ...
        for child in self.__tree.children(node.identifier):
            try:
                ...
                if x[
                    node.data.feature_split
                ] == node.data.feature_label_split and child.tag == "is " + str(
                    node.data.feature_label_split
                ):
                    return self.__query(x, child)
                elif x[
                    node.data.feature_split
                ] != node.data.feature_label_split and child.tag == "not " + str(
                    node.data.feature_label_split
                ):
                    return self.__query(x, child)
            else:
                if feature <= node.data.threshold_split and child.tag == "less " + str(
                    node.data.threshold_split
                ):
                    return self.__query(x, child)
                elif (
                    feature > node.data.threshold_split
                    and child.tag == "greater " + str(node.data.threshold_split)
                ):
                    return self.__query(x, child)
