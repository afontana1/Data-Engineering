...
class SVDD:
    ...
    def __score(self, X):
        ...
        for i in range(n_samples):
            ...
            kernel3 = self.__kernel_func(
                self.__X_support, self.__X_support, self.__sigma
            )
            scores[i] = (
                kernel1
                - 2 * self.__a_support.dot(kernel2)
                + self.__a_support.dot(kernel3).dot(self.__a_support.T)
            )
