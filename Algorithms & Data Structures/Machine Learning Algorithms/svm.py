...
class SVM:
    def __qp(self, X, y, kernel, C):
        ...
        self.__bias = y_free[0] - (self.__a_support * self.__y_support).T.dot(
            kernel[support_items, free_items[0]]
        )
            ...
            for i in range(n_samples):
                ...
                if (y[i] * hi < 1 and alpha[i] < C) or (y[i] * hi > 1 and alpha[i] > 0):
                    ...
                    bi = (
                        self.__bias
                        - e[i]
                        - y[i] * kernel[i, i] * (alpha_i - alpha[i])
                        - y[j] * kernel[i, j] * (alpha_j - alpha[j])
                    )
                    bj = (
                        self.__bias
                        - e[j]
                        - y[i] * kernel[i, j] * (alpha_i - alpha[i])
                        - y[j] * kernel[j, j] * (alpha_j - alpha[j])
                    )
