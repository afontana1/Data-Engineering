...
class Diana:
    def fit(self, X, n_clusters):
        ...
        while True:
            cluster_diameters = [
                np.max(distances[cluster][:, cluster]) for cluster in clusters
            ]
            max_diameter_cluster = np.argmax(cluster_diameters)

            max_diff_index = np.argmax(
                np.mean(
                    distances[clusters[max_diameter_cluster]][
                        :, clusters[max_diameter_cluster]
                    ],
                    axis=1,
                )
            )
            ...
            while True:
                ...
                for j in range(len(old_party))[::-1]:
                    ...
                    distances_old = distances[
                        old_party[j], np.delete(old_party, j, axis=0)
                    ]
