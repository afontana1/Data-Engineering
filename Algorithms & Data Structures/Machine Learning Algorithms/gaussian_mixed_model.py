...
class GMM:
    def fit(self, X, n_clusters, epochs):
        ...
        for _ in range(epochs):
            ...
            for i in range(self.__n_clusters):
                self.__means[i] = (
                    np.sum(y_probs[:, i].reshape((-1, 1)) * X, axis=0) / n_classes[i]
                )

                diff1 = (X - self.__means[i])[:, :, np.newaxis]
                diff2 = np.transpose(diff1, axes=(0, 2, 1)) * y_probs[:, i].reshape(
                    -1, 1, 1
                )
                self.__sigma[i] = (
                    np.tensordot(diff1, diff2, axes=(0, 0)).reshape(
    ...
    def score(self, X):
        ...
        for i in range(self.__n_clusters):
            X_probs[:, i] = multivariate_normal.pdf(
                X, mean=self.__means[i], cov=self.__sigma[i]
            )

        return (
            self.__phi * X_probs / np.sum(self.__phi * X_probs, axis=1, keepdims=True)
        )
