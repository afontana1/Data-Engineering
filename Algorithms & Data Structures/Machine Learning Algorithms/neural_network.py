...
class Conv1d:
    ...
    def backward(self, residual):
        residual = np.pad(
            residual[:], ((0, 0), (self.__padding, self.__padding), (0, 0)), "constant"
        )
...
class Conv2d:
    ...
    def init(self, input_size=0):
        ...
        self.__output_h = (
            self.__input_h - self.__kernel_size
        ) // self.__stride_size + 1
        self.__output_w = (
            self.__input_w - self.__kernel_size
        ) // self.__stride_size + 1
        ...
        self.__W = self.__weights_initializer(
            self.__kernel_size**2 * self.__input_channels,
            self.__filter_number,
            (
                self.__filter_number,
    ...
    def __img2col(self, img, input_channels):
        col = np.zeros(
            (
                self.__batch_size,
                self.__output_size,
                self.__kernel_size**2 * input_channels,
            )
        )
        for h in range(0, self.__output_h):
            for w in range(0, self.__output_w):
                col[:, self.__output_w * h + w, :] = img[
                    :,
                    :,
                    h * self.__stride_size : h * self.__stride_size
                    + self.__kernel_size,
                    w * self.__stride_size : w * self.__stride_size
                    + self.__kernel_size,
    ...
    def forward(self, X, mode):
        ...
        X = np.pad(
            ...
            (
                (0, 0),
                (0, 0),
                (self.__padding_size, self.__padding_size),
                (self.__padding_size, self.__padding_size),
            ),
            "constant",
        )
            ...
            for w in range(0, self.__output_w):
                output_index = np.argmax(
                    X[
                        :,
                        :,
                        h * self.__pool_size : h * self.__pool_size + self.__pool_size,
                        w * self.__pool_size : w * self.__pool_size + self.__pool_size,
                ...
                for i in range(self.__batch_size):
                    for j in range(self.__input_channels):
                        self.__output_index[
                            i,
                            j,
                            h * self.__pool_size + output_index_h[i, j],
                            w * self.__pool_size + output_index_w[i, j],
                        ] = 1
...
class MeanPool:
    ...
    def forward(self, X, mode):
        ...
        return (
            X[:, :, : H * self.__pool_size, : W * self.__pool_size]
            .reshape(
                self.__batch_size,
                self.__input_channels,
                H,
...
class BatchNormalization:
    ...
    def backward(self, residual):
        ...
        d_var = np.sum(
            -d_X_hat * (self.__X - self.__mean) * ((self.__std + eta) ** -3) / 2, axis=0
        )

        d_mean = np.sum(-d_X_hat / (self.__std + eta), axis=0) - 2 * d_var * np.mean(
            self.__X - self.__mean
        )

        batch_size = self.__X.shape[0]
        return (
            d_X_hat / (self.__std + eta)
...
class Rnn:
    ...
    def init(self, input_size=0):
        ...
        self.__rnn_cells.append(
            self.__RnnCell(self.__input_size, self.output_size, self.__optimizer[0])
        )
        for i in range(1, self.__layer_size):
            self.__rnn_cells.append(
                self.__RnnCell(self.output_size, self.output_size, self.__optimizer[i])
            )
...
class NeuralNetwork:
    ...
    def __get_loss(self, h, y):
        if self.__loss == "binary_crossentropy":
            loss = -np.mean(
                np.sum(y * np.log(h + eta) + (1 - y) * np.log(1 - h + eta), axis=1)
            )
    ...
    def fit(self, X, y, batch_size, epochs):
        ...
        for _ in range(epochs):
            ...
            self.__log(
                _, loss[-1], timeit.default_timer() - start_time, np.mean(accuracy)
            )
