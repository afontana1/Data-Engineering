...
class ID3:
    ...
    def __create_tree(self, parent, X, y):
        ...
        if len(np.unique(y)) == 1 or (X == X[0]).all():
            self.__tree.update_node(
                parent.identifier, data=max(set(y), key=y.tolist().count)
            )
            return
        ...
        for i in range(n_features):
            ...
            y_subs = [
                y[np.flatnonzero(X[:, i] == feature_label)]
                for feature_label in np.unique(X[:, i])
            ]
        ...
        for feature_label in np.unique(X[:, feature_split]):
            ...
            self.__create_tree(
                node,
                X[np.flatnonzero(X[:, feature_split] == feature_label)],
                y[np.flatnonzero(X[:, feature_split] == feature_label)],
            )
