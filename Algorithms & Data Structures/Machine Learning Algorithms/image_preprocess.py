...
def Hog(image, orientations, cells_per_block, pixels_per_cell, stride):
    ...
    cells_hog_height, cells_hog_width = (
        image_height + cell_height - 1
    ) // cell_height, (image_width + cell_width - 1) // cell_width
    ...
    for i in range(cells_hog_height):
        for j in range(cells_hog_width):
            bin_cell = bins[
                cell_height * i : cell_height * i + cell_height,
                cell_width * j : cell_width * j + cell_width,
            ]
            g_cell = gxy[
                cell_height * i : cell_height * i + cell_height,
                cell_width * j : cell_width * j + cell_width,
    ...
    blocks_hog_height, blocks_hog_width = (
        cells_hog_height - block_height
    ) // stride + 1, (cells_hog_width - block_width) // stride + 1
    blocks_hog = np.zeros(
        (blocks_hog_height, blocks_hog_width, orientations * block_height * block_width)
    )
    for i in range(blocks_hog_height):
        for j in range(blocks_hog_width):
            block = cells_hog[
                stride * i : stride * i + block_height :,
                stride * j : stride * j + block_width,
            ].ravel()
