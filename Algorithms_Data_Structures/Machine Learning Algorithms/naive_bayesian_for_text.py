...
class NaiveBayesianForText:
    def fit(self, X, y):
        ...
        for i in range(n_classes):
            word_of_classes[i] += np.sum(
                word_vector[np.flatnonzero(y == self.__classes[i])], axis=0
            )
            word_of_classes_total[i] += np.sum(word_of_classes[i])

        self.__p_word_of_classes = word_of_classes / word_of_classes_total.reshape(
            (-1, 1)
        )
    ...
    def predict(self, X):
        ...
        for i in range(n_samples):
            _, indexes, _ = np.intersect1d(
                self.__model.word_dictionary, X[i], return_indices=True
            )
            word_vector[i, indexes] = 1

        p_class_of_doc = word_vector.dot(np.log(self.__p_word_of_classes).T) + np.log(
            self.__p_classes
        )
