...
class C45:
    ...
    def __get_info_gain(self, X_subs, y_subs, y):
        return self.__get_entropy(y) - sum(
            [self.__get_entropy(y_sub) * len(y_sub) for y_sub in y_subs]
        ) / len(y)
    ...
    def __process_discrete(self, x, y):
        y_subs = [
            y[np.flatnonzero(x == feature_label)] for feature_label in np.unique(x)
        ]
    ...
    def __create_tree(self, parent, X, y):
        ...
        if data.threshold_split:
            ...
            for feature_label in np.unique(X[:, data.feature_split]):
                ...
                self.__create_tree(
                    node,
                    X[np.flatnonzero(X[:, data.feature_split] == feature_label)],
                    y[np.flatnonzero(X[:, data.feature_split] == feature_label)],
                )
            ...
            for node in self.__tree.all_nodes():
                ...
                if self.__tree.level(node.identifier) == level and not node.is_leaf():
                    ...
                    leaves_error = sum(
                        [
                            leaf.data.n_errors
                            for leaf in self.__tree.leaves(node.identifier)
                        ]
                    )
    ...
    def predict(self, X):
        ...
        return np.apply_along_axis(
            self.__query, 1, X, self.__tree.get_node(self.__tree.root)
        )
