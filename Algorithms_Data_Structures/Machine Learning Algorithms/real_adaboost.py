...
class Weaker:
    ...
    def fit(self, X, y, w):
        ...
        for i in range(n_features):
            ...
            for k in range(self.__n_subsets):
                subset_indexes = np.flatnonzero(
                    (split_points[k] < X[:, i]) & (X[:, i] <= split_points[k + 1])
                )
                w_positive[k] = sum(
                    w[subset_indexes][np.flatnonzero(y[subset_indexes] == 1)]
                )
                w_negative[k] = sum(
                    w[subset_indexes][np.flatnonzero(y[subset_indexes] == -1)]
                )
    ...
    def predict(self, X):
        ...
        for i in range(n_samples):
            for k in range(self.__n_subsets):
                if (
                    self.__split_points[k]
                    < X[i, self.__feature]
                    <= self.__split_points[k + 1]
                ):
                    h[i] = self.__h[k]
                    break
