...
def confusion_matrix(y_true, y_pred):
    ...
    for r in range(n_classes):
        for c in range(n_classes):
            matrix[r, c] = np.sum(
                y_pred[np.flatnonzero(y_true == classes[r])] == classes[c]
            )
...
def silhouette_coefficient(X, y, distance):
    ...
    for i in range(n_samples):
        ...
        for cluster in np.unique(y):
            if y[i] == cluster:
                a = np.sum(distances[np.flatnonzero(y == cluster)]) / (
                    np.sum(y == cluster) - 1 + 1e-8
                )
            else:
...
def information_value(X, y):
    ...
    for i in range(n_features):
        ...
        for feature_label in feature_labels:
            ...
            iv += (p_positive - p_negative) * np.log(
                p_positive + 1e-8 / p_negative + 1e-8
            )
