...
class IsolateTree:
    ...
    def __create_tree(self, parent, X):
        ...
        data.threshold_split = (
            max(X[:, data.feature_split]) - min(X[:, data.feature_split])
        ) * np.random.random() + min(X[:, data.feature_split])
    ...
    def __get_path_length(self, x, node):
        if node.is_leaf():
            return self.__tree.depth(node.identifier) + (
                self.__get_normalization_factor(node.data.n_samples)
                if node.data.n_samples > 1
                else 0
            )
        ...
        for child in self.__tree.children(node.identifier):
            ...
            ] < node.data.threshold_split and child.tag == "less " + str(
                node.data.threshold_split
            ):
                return self.__get_path_length(x, child)
            elif x[
                node.data.feature_split
            ] >= node.data.threshold_split and child.tag == "greater " + str(
                node.data.threshold_split
            ):
                return self.__get_path_length(x, child)
    ...
    def get_abnormal_score(self, X):
        return 2 ** (
            -np.apply_along_axis(
                self.__get_path_length, 1, X, self.__tree.get_node(self.__tree.root)
            )
            / self.__c
        )
