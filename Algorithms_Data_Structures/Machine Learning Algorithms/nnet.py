...
class Conv1d:
    def __init__(
        ...
        padding="same",
        input_size=0,
        weights_initializer=weights_initializer.he_normal,
    ):
    ...
    def init(self, optimizer, learning_rate, input_size=0):
        ...
        if self.__padding == "same":
            self.__padding_size = (
                (math.ceil(self.__input_timesteps / self.__stride_size) - 1)
                * self.__stride_size
                + self.__kernel_size
                - self.__input_timesteps
            )
        ...
        self.__output_size = (
            self.__input_timesteps - self.__kernel_size
        ) // self.__stride_size + 1

        self.output_size = (self.__output_size, self.__filter_number)

        self.__W = self.__weights_initializer(
            self.__kernel_size * self.__input_channels,
            self.__filter_number,
            (self.__filter_number, self.__input_channels, self.__kernel_size),
        )
    ...
    def __img2col(self, img, input_channels):
        col = np.zeros(
            (self.__batch_size, self.__output_size, self.__kernel_size * input_channels)
        )
        for h in range(0, self.__output_size):
            col[:, h, :] = img[
                :,
                h * self.__stride_size : h * self.__stride_size + self.__kernel_size,
                :,
            ].reshape(self.__batch_size, -1, order="F")
    ...
    def forward(self, X, mode):
        ...
        if self.__padding == "same":
            X = np.pad(
                X[:],
                ((0, 0), (self.__padding_pre, self.__padding_latter), (0, 0)),
                "constant",
            )

        self.__col = self.__img2col(X, self.__input_channels)
        return (
            self.__col.dot(self.__W.reshape(self.__filter_number, -1).T)
            + self.__b[None, None, :]
        )
    ...
    def optimize(self, residual):
        g_W = (
            np.tensordot(
                residual.reshape(self.__batch_size, self.__filter_number, -1),
                self.__col,
                axes=[[0, 2], [0, 1]],
            )
            / self.__batch_size
        ).reshape(self.__W.shape)
...
class Conv2d:
    ...
    def init(self, optimizer, learning_rate, input_size=0):
        ...
        if self.__padding == "same":
            self.__padding_h = (
                (math.ceil(self.__input_h / self.__stride_h) - 1) * self.__stride_h
                + self.__kernel_h
                - self.__input_h
            )
            self.__padding_w = (
                (math.ceil(self.__input_w / self.__stride_w) - 1) * self.__stride_w
                + self.__kernel_w
                - self.__input_w
            )
    ...
    def __img2col(self, img, input_channels):
        ...
        for h in range(0, self.__output_h):
            for w in range(0, self.__output_w):
                col[:, self.__output_w * h + w, :] = img[
                    :,
                    :,
                    h * self.__stride_h : h * self.__stride_h + self.__kernel_h,
                    w * self.__stride_w : w * self.__stride_w + self.__kernel_w,
    ...
    def forward(self, X, mode):
        ...
        if self.__padding == "same":
            X = np.pad(
                ...
                (
                    (0, 0),
                    (0, 0),
                    (self.__padding_h_up, self.__padding_h_down),
                    (self.__padding_w_left, self.__padding_w_right),
                ),
                "constant",
            )
        ...
        return (
            np.transpose(output, axes=(0, 2, 1)).reshape(
                (
                    self.__batch_size,
                    self.__filter_number,
                    self.__output_h,
...
class MaxPool:
    ...
    def forward(self, X, mode):
        ...
        output = np.zeros(
            (self.__batch_size, self.__input_channels, self.__output_h, self.__output_w)
        )
        self.__output_index = np.zeros_like(X)
        for h in range(0, self.__output_h):
            for w in range(0, self.__output_w):
                output_index = np.argmax(
                    X[
                        :,
                        :,
                        h * self.__stride_h : h * self.__stride_h + self.__pool_h,
                        w * self.__stride_w : w * self.__stride_w + self.__pool_w,
                    ].reshape((self.__batch_size, self.__input_channels, -1)),
                    axis=2,
                )
                ...
                for i in range(self.__batch_size):
                    for j in range(self.__input_channels):
                        self.__output_index[
                            i,
                            j,
                            h * self.__stride_h + output_index_h[i, j],
                            w * self.__stride_w + output_index_w[i, j],
                        ] = 1
                output[:, :, h, w] = np.max(
                    X[
                        :,
...
class MeanPool:
    ...
    def forward(self, X, mode):
        ...
        return (
            X[:, :, : MK * K, : NL * L]
            .reshape(X.shape[0], X.shape[1], MK, K, NL, L)
            .mean(axis=(3, 5))
        )
        """
...
class Dropout:
    ...
    def forward(self, X, mode):
        if self.__p > 0 and mode == "fit":
            self.__dropout_index = random.sample(
                range(self.output_size), int(self.output_size * self.__p)
            )
...
class BatchNormalization:
    ...
    def forward(self, X, mode):
        ...
        if mode == "fit":
            ...
            self.__predict_mean = (
                momentum * self.__predict_mean + (1 - momentum) * self.__mean
            )

            self.__std = np.std(self.__X, axis=0)
            self.__predict_std = (
                momentum * self.__predict_std + (1 - momentum) * self.__std
            )

            self.__X_hat = (self.__X - self.__mean) / (self.__std + 1e-8)
        elif mode == "predict":
            self.__X_hat = (self.__X - self.__predict_mean) / (
                self.__predict_std + 1e-8
            )
    ...
    def backward(self, residual):
        ...
        d_var = np.sum(
            -d_X_hat * (self.__X - self.__mean) * ((self.__std + 1e-8) ** -3) / 2,
            axis=0,
        )

        d_mean = np.sum(-d_X_hat / (self.__std + 1e-8), axis=0) - 2 * d_var * np.mean(
            self.__X - self.__mean
        )

        batch_size = self.__X.shape[0]
        return (
            d_X_hat / (self.__std + 1e-8)
            + d_var * 2 * (self.__X - self.__mean) / batch_size
            + d_mean / batch_size
        )
...
class Rnn:
    class __RnnCell:
        def __init__(self, input_size, output_size, optimizer):
            self.__U = weights_initializer.xavier_normal(
                output_size, output_size, (output_size, output_size)
            )
            self.__U_gradient = 0
            self.__W = weights_initializer.xavier_normal(
            ...
            self.__V = weights_initializer.xavier_normal(
                output_size, output_size, (output_size, output_size)
            )
            self.__V_gradient = 0
        ...
        def forward(self, X, h_pre):
            ...
            self.__h = np.tanh(
                self.__X.dot(self.__W) + self.__h_pre.dot(self.__U) + self.__b
            )
        ...
        def optimize(self):
            g_U, g_W, g_b, g_V, g_c = self.__optimizer.optimize(
                [
                    self.__U_gradient,
                    self.__W_gradient,
                    self.__b_gradient,
    ...
    def __init__(self, time_steps, output_size, activivation, layer_size, input_size=0):
        ...
        self.__activations = np.array(
            [activivation() for i in range(self.__time_steps * self.__layer_size)]
        ).reshape((self.__layer_size, self.__time_steps))
    ...
    def init(self, optimizer, learning_rate, input_size=0):
        ...
        self.__rnn_cells = []
        self.__rnn_cells.append(
            self.__RnnCell(
                self.__input_size, self.output_size, optimizer(learning_rate)
            )
        )
        for i in range(1, self.__layer_size):
            self.__rnn_cells.append(
                self.__RnnCell(
                    self.output_size, self.output_size, optimizer(learning_rate)
                )
            )
    ...
    def forward(self, X, mode):
        ...
        h = np.zeros(
            (batch_size, self.__time_steps, self.__layer_size, self.output_size)
        )
    ...
    def optimize(self, residual):
        ...
        for i in range(self.__time_steps - 1, -1, -1):
            for j in range(self.__layer_size - 1, -1, -1):
                residual_tmp = self.__activations[j, i].backward(
                    residual_X + residual_h[j]
                )
...
class Dense:
    ...
    def init(self, optimizer, learning_rate, input_size=0):
        ...
        self.__W = self.__weights_initializer(
            self.__input_size, self.output_size, (self.__input_size, self.output_size)
        )
    ...
    def optimize(self, residual):
        batch_size = self.__X.shape[0]
        g_W = self.__X.T.dot(residual) / batch_size + self.__regularizer.regularize(
            self.__W
        )
...
class Selu:
    ...
    def forward(self, X, mode):
        self.__X = X
        return self.__scale * np.where(
            self.__X > 0.0, self.__X, self.__alpha * (np.exp(self.__X) - 1)
        )

    def backward(self, residual):
        return (
            self.__scale
            * np.where(self.__X > 0.0, 1, self.__alpha * (np.exp(self.__X) - 1))
            * residual
        )
...
class Nnet:
    ...
    def __get_residual(self, h, y):
        if (
            ...
            or self.__loss == "mse"
            or self.__loss == "categorical_crossentropy"
        ):
            return h - y
            ...
            residual[
                np.flatnonzero(
                    h - h[range(batch_size), correct_index].reshape((-1, 1)) + 1 > 0
                )
            ] = 1
    ...
    def __get_accuracy(self, h, y):
        if (
            ...
            or self.__loss == "categorical_hinge"
        ):
    ...
    def __get_loss(self, h, y):
        if self.__loss == "binary_crossentropy":
            ...
            loss = np.mean(
                np.sum(
                    np.maximum(
                        0, h - h[range(batch_size), correct_index].reshape((-1, 1)) + 1
                    ),
                    axis=1,
                )
                - 1
    ...
    def __log(self, epoch, loss, elapse, accuracy):
        print(
            "epochs: %d; loss: %f; elapse: %f; accuracy: %f"
            % (epoch, loss, elapse, accuracy)
        )
    ...
    def fit(self, X, y, batch_size, epochs):
        ...
        for _ in range(epochs):
            ...
            for i in range(epoch):
                X_batch = X_epoch[
                    batch_size * i : min(batch_size * (i + 1), data_number)
                ]
                y_batch = y_epoch[
                    batch_size * i : min(batch_size * (i + 1), data_number)
                ]
