...
class Momentum:
    ...
    def optimize(self, vars):
        ...
        self.__v = [
            (1 - self.__alpha) * vars[i] + self.__alpha * self.__v[i]
...
class Nesterov:
    ...
    def optimize(self, vars):
        ...
        old_v = self.__v
        self.__v = [
            self.__alpha * self.__v[i] - self.__learning_rate * vars[i]
            for i in range(n_vars)
        ]
...
class Adam:
    ...
    def optimize(self, vars):
        ...
        for i in range(n_vars):
            ...
            self.__r[i] = self.__alpha2 * self.__r[i] + (1 - self.__alpha2) * np.power(
                vars[i], 2
            )
