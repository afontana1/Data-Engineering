...
def test_get_conversation(target):
    ...
    data = {
        "file1.json": {"log": [{"text": "hi"}, {"text": "hello"}, {"text": "nice"}]},
        "file2.json": {
            "log": [{"text": "a b"}, {"text": ""}, {"text": "good "}, {"text": "no?"}]
        },
    }
    ...
    for test_case in test_cases:
        try:
            if test_case["name"] == "simple_test_check":
                assert test_case["expected"] == str(temp_model).replace(
                    " ", ""
                ).replace("\n", "")
                success += 1
...
def test_tasks(train_task, eval_task):
    ...
    try:
        ...
        fails += 1
        print(
            f"Wrong metrics. found {strlabel} but expected [CrossEntropyLoss_in3,Accuracy_in3]"
        )
