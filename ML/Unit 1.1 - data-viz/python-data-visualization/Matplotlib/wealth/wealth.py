...
df = (
    df[["Net worth", "Category", "Datex"]]
    .groupby(["Datex", "Category"])
    .sum()
    .unstack()
)
df = df.apply(lambda x: x / x.sum() * 100, axis=1)
df.loc[:, ("Net worth", "Top10")] = df["Net worth"]["Top1"] + df["Net worth"]["Next9"]
df.loc[:, ("Net worth", "Bottom90")] = (
    df["Net worth"]["Next40"] + df["Net worth"]["Bottom50"]
)
df = df.stack()
df = df.reset_index()
...
COLORS = {"Top10": "#912d00", "Bottom90": "#006595"}
...
plt.title(
    "The U.S. Household Wealth Distribution", fontsize=28, loc="left", x=-0.08, y=1.14
)

ax.text(
    s="The 10 percent wealthiest households possess almost \n70 percent of the total household wealth",
    x=-0.08,
    y=1.06,
    fontsize=18,
