...
for (cyl, group), c in zip(plot_df.groupby("cylinders"), colors):
    ...
    density_ax = ax.inset_axes(
        [0, cyl + box_width * 0.5, 1, 2 - box_width * 1.5],
        transform=transform,
        sharex=ax,
    )
    density_ax.axis("off")
    ...
    kdeplot(
        group, x="mpg", ax=density_ax, fill=True, bw_adjust=0.25, alpha=0.5, color=c
    )

    transform = offset_copy(ax.get_yaxis_transform(), y=-5, fig=fig, units="points")
    hist_ax = ax.inset_axes(
        [0, cyl - 2 + box_width, 1, 2 - 1.5 * box_width], transform=transform, sharex=ax
    )
    hist_ax.axis("off")
    ...
    segments = column_stack([icicles.index, starts, icicles.index, icicles]).reshape(
        -1, 2, 2
    )
