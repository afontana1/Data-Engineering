...
def download_from_github(version, fn, target_dir):
    url = "https://github.com/hse-aml/intro-to-dl/releases/download/{0}/{1}".format(
        version, fn
    )
...
def download_all_keras_resources(keras_models, keras_datasets):
    ...
    sequential_downloader(
        "v0.2", ["cifar-10-batches-py.tar.gz", "mnist.npz"], keras_datasets
    )
...
def download_week_3_resources(save_path):
    ...
    sequential_downloader("v0.3", ["102flowers.tgz", "imagelabels.mat"], save_path)
...
def download_week_4_resources(save_path):
    ...
    sequential_downloader(
        "v0.4", ["lfw-deepfunneled.tgz", "lfw.tgz", "lfw_attributes.txt"], save_path
    )
...
def link_all_keras_resources():
    link_all_files_from_dir(
        "../readonly/keras/datasets/", os.path.expanduser("~/.keras/datasets")
    )
    link_all_files_from_dir(
        "../readonly/keras/models/", os.path.expanduser("~/.keras/models")
    )
