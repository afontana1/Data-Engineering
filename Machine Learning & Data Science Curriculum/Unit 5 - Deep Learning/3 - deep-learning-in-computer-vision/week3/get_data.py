...
def download(
    filename, source="https://github.com/vslutov/face-detection/releases/download/v1.2/"
):
...
def load_dataset(path, dname):
    ...
    # Images
    with open(
        os.path.join("data", "{dname}_fnames.csv".format(dname=dname))
    ) as fnames_fin:
    ...
    return (
        ...
        np.array(image_shapes, dtype=np.int32),
    )
