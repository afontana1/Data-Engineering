...
COVER_IMAGE = (
    "https://raw.githubusercontent.com/ipython-books/"
...
def process_urls(contents):
    contents = re.sub(
        r"\[([^\]]+)\]\(([^\)\{]+\.md)\)", r"[\1]({filename}\2)", contents
    )
    contents = re.sub(
        r"\[([^\]]+)\]\(([^\)\/\{]+)\)", r"[\1]({filename}\2/index.md)", contents
    )
    return contents
...
def process_ad(contents, path):
    contents = contents.replace(
        '<a href="https://github.com/ipython-books/cookbook-2nd">'
...
def create_index():
    ...
    readme = readme.replace(
        "This repository contains the sources of the book (in Markdown, ",
...
def write_file(fin, fout):
    ...
    path = (
        "https://github.com/ipython-books/cookbook-2nd-code/blob/master/" "%s/%s"
    ) % (chapter, name)
