...
def run_env_check():
    ...
    for component, value in zip(
        ...
        if getattr(python_version, component) != int(value):
            print(
                FAIL,
                "Python version %s is required, but %s is installed.\n"
        ...
        try:
            ...
            if req_version:
                ...
                if Version(version) != Version(req_version):
                    print(
                        FAIL,
                        "%s version %s is required, but %s installed."
                        % (pkg, req_version, version),
                    )
                    continue
            print(OK, "%s" % pkg)
        except ImportError:
            if pkg == "ffmpeg":
                try:
                    pkg_info = json.loads(
                        os.popen("conda list -f ffmpeg --json").read()
                    )[0]
                    if pkg_info:
                        if pkg_info["version"] != req_version:
                            print(
                                FAIL,
                                "%s version %s is required, but %s installed."
                                % (pkg, req_version, pkg_info["version"]),
                            )
                            continue
                        print(OK, "%s" % pkg)
                        continue
