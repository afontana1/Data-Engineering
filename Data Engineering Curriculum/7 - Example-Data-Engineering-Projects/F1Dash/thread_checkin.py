...
def thread_checkin(type, host_name, thread_id):
    ...
    return_val = cursor.execute(
        "SET NOCOUNT ON; EXEC dbo.Thread_Checkin @CheckinType=?, @HostName=?, @ThreadId=?",
        param_values,
    ).fetchval()
    cursor.commit()
...
def thread_loop(
    type, max_wakeup_delay, thread_sleep_in_hours, delete_delay_in_hours=None
):
    ...
    read_database.app_logging(
        "app", "thread", f"Initiated {type} thread id {thread_id}"
    )
        ...
        if thread_checkin(type, host_name, thread_id):
            ...
            if type == "Database":
                read_database.app_logging(
                    "app",
                    "database_thread",
                    f"Running database thread loop id {thread_id}",
                )
                quick_loop = update_database.wrapper()
            elif type == "Cache":
                read_database.app_logging(
                    "app",
                    "cache_cleanup_thread",
                    f"Running cache cleanup thread loop id {thread_id}",
                )
                ...
                if files_deleted > 0:
                    read_database.app_logging(
                        "app",
                        "cache_cleanup_thread",
                        f"Cache cleanup thread deleted {files_deleted} files",
                    )
                quick_loop = False
