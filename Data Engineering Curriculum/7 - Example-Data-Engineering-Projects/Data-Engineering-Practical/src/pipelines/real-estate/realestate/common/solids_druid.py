...
def ingest_druid(
    ...
    if r.status_code != 200:
        ...
        intervalToDelete = json.dumps(
            {"interval": druid_coordinate["intervalToDelete"]}
        )
        ...
        #
        try:
            spec_path = file_relative_path(
                __file__, druid_coordinate["PathToJsonIngestSpec"]
            )
        ...
        context.log.info(
            "3. Ingest spec successfully loaded from path '{fpath}'".format(
                fpath=spec_path
            )
        )
