...
def main():
    ...
    unit_prices_with_ratings = prices_with_ratings.withColumn(
        "unit_price_in_euro", col("price") / col("quantity")
    )

    (
        unit_prices_with_ratings.filter(
            (col("absorption_rate") >= 4) & (col("comfort") >= 3)
        )
        .orderBy(col("unit_price_in_euro").asc())
        .limit(10)
        .repartition(1)
        .write.csv(catalog["business/top_n_diapers"])
    )
